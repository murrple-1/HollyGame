...
public class Player extends Actor {
	...
	private static Array<Animation<TextureRegion>> animations = null;
	...
	private static class CheckKillsAction extends Action {
		...
		public boolean act(float delta) {
			...
			Collection<Enemy> destroyedEnemies = new ArrayList<>();
	...
	public void draw(Batch batch, float parentAlpha) {
		...
		Animation<TextureRegion> a = animations.get(direction.ordinal() + step);
	...
	private static void createPlayerAnimations(MyGdxGame game) {
		if(animations == null) {
			...
			animations.setSize(PlayerDirections.length * 3);
			...
			for(int i = 0; i < PlayerDirections.length; i++) {
				...
				Animation<TextureRegion> a = new Animation<>(PLAYER_FRAME_DURATION, new Array<>(frames), Animation.PlayMode.LOOP);
				animations.set(i, a);
			}
			for(int i = 0; i < PlayerDirections.length; i++) {
				PlayerDirection d = PlayerDirections[i];
				...
				Animation<TextureRegion> a = new Animation<>(PLAYER_FRAME_DURATION, new Array<>(frames), Animation.PlayMode.LOOP);
				animations.set(i + PlayerDirections.length, a);
			}
			for(int i = 0; i < PlayerDirections.length; i++) {
				PlayerDirection d = PlayerDirections[i];
				char dChar = playerDirectionToChar(d);
				...
				Animation<TextureRegion> a = new Animation<>(PLAYER_FRAME_DURATION, new Array<>(frames), Animation.PlayMode.LOOP);
				animations.set(i + PlayerDirections.length + PlayerDirections.length, a);
			}
